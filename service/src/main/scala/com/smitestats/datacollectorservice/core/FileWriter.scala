...
import com.smitestats.datacollectorservice.models.{ GetMatchDetailsBatchResponse, MatchDataOutput }
...
object FileWriter {
    ...
    def fileName: IO[String] = IO { s"matchData_${getHourTwoHoursPrev}.csv"}

    def writeFile(items: List[List[GetMatchDetailsBatchResponse]]): IO[MatchDataOutput] = {
        for {
            name <- fileName
            _ <- IO { logger.info(s"Building file ${name}...") }  
            content <- Stream.emits(items)
                .through(toCSVRow)
                .compile
                .string               
        } yield MatchDataOutput(name, headerRow ++: content)
    }
